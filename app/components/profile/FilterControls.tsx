    ...
    <div className="bg-white shadow rounded-lg p-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
        ...
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 flex-1">
          ...
          {/* Pass/Fail Filter */}
          <div className="min-w-0">
            <label htmlFor="passed" className="sr-only">
              Filter by pass status
            </label>
            <select
              id="passed"
              value={filters.passed}
              onChange={(e) => onFilterChange({ passed: e.target.value })}
              className="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
            >
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {filters.passed && (
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
              Status: {filters.passed === 'true' ? 'Passed' : 'Failed'}
              <button
          ...
          {filters.dateRange && (
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
              Time: {filters.dateRange === 'week' ? 'Last Week' : 
              ...
              <button
                ...
                className="ml-1.5 inline-flex items-center justify-center w-4 h-4 rounded-full text-orange-400 hover:bg-orange-200 hover:text-orange-600 focus:outline-none"
              >
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
